# -*- coding: utf-8 -*-
"""
Created on Tue Jan  8 01:07:48 2019
...
# Load encoder
encoder = joblib.load(os.path.join(data_path,'models_scalers','eyeglasses_encoder.pkl'))

# Load sgd optimizer
sgd = optimizers.SGD(lr=0.001, decay=1e-3, momentum=0.9, nesterov=True)

# load json and create model
json_file = open(os.path.join(data_path,'models_scalers','eyeglasses_model.json'), 'r')
loaded_model_json = json_file.read()
json_file.close()
loaded_model = model_from_json(loaded_model_json)
# load weights into new model
loaded_model.load_weights(os.path.join(data_path,'models_scalers','best_model_eyeglasses.h5'))
...
csv_file.to_csv(os.path.join(data_path,'test_results','task_3.csv'),header=[acc,''],index=False)
