# -*- coding: utf-8 -*-
"""
Created on Mon Jan  7 22:00:25 2019
...
label = 'hair_color' #choose from 'smiling', 'eyeglasses', 'human', 'young', or 'hair_color'
...
train_label = pd.read_csv(os.path.join(data_path,'train_test_csv','hair_color_train.csv'),header = 0,index_col=0)[label]
test_label = pd.read_csv(os.path.join(data_path,'train_test_csv','hair_color_val.csv'),header = 0,index_col=0)[label]
train_label = pd.concat([train_label,test_label])

# Fit the encoder and save it
encoder.fit(np.array(train_label).reshape(-1,1))
joblib.dump(encoder, os.path.join(model_path,('hair_color_encoder.pkl')))
...
callbacks = [EarlyStopping(monitor='val_loss', patience=5),ModelCheckpoint(filepath=os.path.join(model_path,'best_model_hair_color.h5'), monitor='val_loss', save_best_only=True)]
...
with open(os.path.join(model_path,"hair_color_model.json"), "w") as json_file:
    json_file.write(model_json)
# serialize weights to HDF5
model.save_weights(os.path.join(model_path,"hair_color_model.h5"))
