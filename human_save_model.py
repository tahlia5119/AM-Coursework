# -*- coding: utf-8 -*-
"""
Created on Mon Jan  7 21:58:31 2019
...
label = 'human' #choose from 'smiling', 'eyeglasses', 'human', 'young', or 'hair_color'
...
train_label = pd.read_csv(os.path.join(data_path,'train_test_csv','human_train.csv'),header = 0,index_col=0)['human']
test_label = pd.read_csv(os.path.join(data_path,'train_test_csv','human_val.csv'),header = 0,index_col=0)['human']
train_label = pd.concat([train_label,test_label])

# Fit the encoder and save it
encoder.fit(np.array(train_label).reshape(-1,1))
joblib.dump(encoder, os.path.join(model_path,('human_encoder.pkl')))
...
callbacks = [EarlyStopping(monitor='val_loss', patience=5),ModelCheckpoint(filepath=os.path.join(model_path,'best_model_human.h5'), monitor='val_loss', save_best_only=True)]
...
with open(os.path.join(model_path,"human_model.json"), "w") as json_file:
    json_file.write(model_json)
# serialize weights to HDF5
model.save_weights(os.path.join(model_path,"human_model.h5"))